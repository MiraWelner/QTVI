function [rr] = JoinedRR(ecgSeg,ecgSamplingRate,diff_range, fileID)
    if std(ecgSeg) == 0
        return
    end
    
    algos = {{@rpeakdetect, {ecgSeg,ecgSamplingRate,  0.2, 0, fileID}}, ...
            {@rpeakdetect, {ecgSeg,ecgSamplingRate,0.1, 0, fileID}}, ...
            {@rpeakdetect, {ecgSeg,ecgSamplingRate,0.4, 0, fileID}},...
    ...
    parfor i=1:length(algos)
        ...
        try
            ...
            output{i} = curr_alg{1}(curr_alg{2}{1},curr_alg{2}{2},curr_alg{2}{3},curr_alg{2}{4},curr_alg{2}{5});            end
        catch
            output{i} = [];
%             disp(':(');
        end
