...
if dbg == 1
    figure;
    ax1 = subplot(511); 
    plot(origecg);hold on; plot(rneg,origecg(rneg),'o');xlabel('samples');ylabel('ECG (mV)');title('original ecg with rneg');
    ax2 =subplot(512); 
    plot(origecg);hold on; plot(rpos,origecg(rpos),'*');xlabel('samples');ylabel('ECG (mV)');title('original ecg with rpos');
    ax3 =subplot(513); 
    plot(filtecg);xlabel('samples');ylabel('ECG (mV)');title('filtecg ecg');
    ax4 =subplot(514); 
    plot(difffiltecg);xlabel('samples');ylabel('ECG (mV)');title('difffiltecg ecg');
    ax5 =subplot(515); 
    plot(mwisignal);xlabel('samples');ylabel('ECG (mV)');title('mwisignal');
