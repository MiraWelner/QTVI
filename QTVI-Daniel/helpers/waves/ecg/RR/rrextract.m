function [rr, rs, RtR] = rrextract(ecg, sf, thresh, testmode, twind)
    % [rr rs RtR] = rrextract(ecg, sf, thresh, testmode, twind) extracts rr times
    % and rs amplitudes from ecg with sampling frequency sf using rpeakdetect.m
    %R to R times calculated and returned in variable 'RtR'
    ...
    %R to R time calculation %M. Hautman
    RtR = diff(rr) * (1 / sf);
end
