function [template, bad_segment_fig] = EnsembleTemplate(wave, segment_idxs, std_multiplier, type, dbg_plot, fig_name, expand)
    if exist('expand','var') == 1
        segment_idxs = [segment_idxs(1:end - 1) segment_idxs(2:end)];
        segment_idxs(:,1) = segment_idxs(:,1) - expand;
        segment_idxs(:,2) = segment_idxs(:,2) + expand;
        segment_idxs(segment_idxs(:,1)<1,1) = 1;
        segment_idxs(segment_idxs(:,2) > length(wave),2) = length(wave);
