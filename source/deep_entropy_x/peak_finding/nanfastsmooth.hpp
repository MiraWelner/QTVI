...
namespace nanfastsmooth_detail {
    ...
    inline vector<double> sa(const vector<double>& Y, int smoothwidth, double tol) {
        ...
        if (w % 2 == 1) {
            ...
            for (int i = 0; i <= halfw && i < static_cast<int>(L); ++i) {
                if (!std::isnan(Y[i])) { SumPoints += Y[i]; NumPoints += 1.0; }
            }
            ...
            for (size_t k = 1; k < L; ++k) {
                ...
                if (removeIdx >= 0 && !std::isnan(Y[removeIdx])) {
                    SumPoints -= Y[removeIdx]; NumPoints -= 1.0;
                }
                if (addIdx < static_cast<int>(L) && !std::isnan(Y[addIdx])) {
                    SumPoints += Y[addIdx]; NumPoints += 1.0;
                }
            ...
            if (halfw < static_cast<int>(L) && !std::isnan(Y[halfw])) {
                SumPoints += 0.5 * Y[halfw]; NumPoints += 0.5;
            }
            ...
            for (size_t k = 1; k < L; ++k) {
                ...
                if (removeIdx1 >= 0 && !std::isnan(Y[removeIdx1])) {
                    SumPoints -= 0.5 * Y[removeIdx1]; NumPoints -= 0.5;
                }
                if (removeIdx2 >= 0 && !std::isnan(Y[removeIdx2])) {
                    SumPoints -= 0.5 * Y[removeIdx2]; NumPoints -= 0.5;
                }
                if (addIdx1 < static_cast<int>(L) && !std::isnan(Y[addIdx1])) {
                    SumPoints += 0.5 * Y[addIdx1]; NumPoints += 0.5;
                }
                if (addIdx2 < static_cast<int>(L) && !std::isnan(Y[addIdx2])) {
                    SumPoints += 0.5 * Y[addIdx2]; NumPoints += 0.5;
                }
...
inline vector<double> nanfastsmooth(const vector<double>& Y, double w, int type = 1, double tol = 0.5) {
    using nanfastsmooth_detail::sa;
