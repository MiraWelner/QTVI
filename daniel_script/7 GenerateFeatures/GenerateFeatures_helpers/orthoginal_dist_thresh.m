function [result] = orthoginal_dist_thresh(time, straight_line, line2, thresh)
    result = 0;

    for x = 2:length(time)
        d = point_to_line_distance([time(x) line2(x) 0], ...
                                   [time(x-1) straight_line(x-1) 0], ...
                                   [time(x) straight_line(x) 0]);
        if d > thresh
            result = 1;
            break
        end
