...
void compareVectors(const std::string& label, const std::vector<double>& bin, const std::vector<double>& mat) {
    ...
    size_t minSize = std::min(bin.size(), mat.size());
    for (size_t i = 0; i < minSize; ++i) {
        if (bin[i] != mat[i]) {
            std::cout << "    ! Value mismatch at index " << i << ": C++=" << bin[i] 
                      << ", MAT=" << mat[i] << std::endl;
        }
    }
}
...
int main() {
    ...
    size_t max_bins = 5;// std::min(binData.size(), matData.size());
