...
# ============================================================================
# Read Deep per-subject folder of Rel*_Abs*.csv files
# ============================================================================
...
def _parse_deep_csv(path):
    """Parse one Rel*_Abs*.csv file. Returns array of times (seconds)."""
    times = []
...
def read_deep_folder(folder_path, n_bins_expected):
    """Load Deep's per-subject folder. Same per-bin dict shape as read_mat_file."""
    empty = {
    ...
    all_times = [_parse_deep_csv(p) for p in csv_paths]

    results = []
    for bin_idx in range(n_bins_expected):
        if bin_idx >= len(all_times) or len(all_times[bin_idx]) == 0:
            results.append(dict(empty))
            continue
        local_samples = np.round(all_times[bin_idx] * SR).astype(np.int64)
        results.append(
            {
                "ecgRIndex": local_samples.astype(np.intp),
                "ecgRIndex_f": local_samples.astype(np.float64),
                "ppgMinAmps": np.array([], dtype=np.intp),
                "ecgSignal": np.array([], dtype=np.float64),
                "ppgSignal": np.array([], dtype=np.float64),
            }
...
def find_deep_folder(subject_id):
    """Locate Deep's per-subject directory by suffix."""
...
def write_summary_csv(path, results, label_a="A", label_b="B"):
    """Pooled ECG summary statistics.

    Two columns: label_a and label_b (e.g. "Mira" and "Daniel"). Drops PPG;
    rows match the user-requested list (subjects, beats, extras, rate,
    interval, amplitude).

    Amplitude stats can be NaN when a source ships peak indices but no
    signal (e.g. Deep). Those values render as blank cells.
    """
    ...
    # Beats present in A but not B (and vice versa). Per-bin sums of the
    # positive count differences across all bins, all subjects.
    ...
    rows = [
        ...
        make_row(
            "Mean Amplitude",
            [
                nanmean(collect(lambda b: b["ecg_amp_cpp"]["mean"])),
                nanmean(collect(lambda b: b["ecg_amp_mat"]["mean"])),
            ],
        ),
        make_row(
            "Median Amplitude",
            [
                nanmedian(collect(lambda b: b["ecg_amp_cpp"]["median"])),
                nanmedian(collect(lambda b: b["ecg_amp_mat"]["median"])),
            ],
        ),
        make_iqr_row(
            "Amplitude IQR",
            [
                nanpercentile(collect(lambda b: b["ecg_amp_cpp"]["q1"]), 25),
                nanpercentile(collect(lambda b: b["ecg_amp_mat"]["q1"]), 25),
            ],
            [
                nanpercentile(collect(lambda b: b["ecg_amp_cpp"]["q3"]), 75),
                nanpercentile(collect(lambda b: b["ecg_amp_mat"]["q3"]), 75),
            ],
        ),
    ]
...
def write_combined_ssd_histogram(path, results, num_buckets=80):
    ...
    for fr in results:
        for b in fr["bins"]:
            if not np.isnan(b["rr_ssd"]):
                all_ssd.append(round(b["rr_ssd"], 6))
    ...
    for b_idx in range(num_buckets):
        ...
        tip = f"[{range_start:.6f}, {range_end:.6f}): {c}"
        lines.append(
...
def write_allfiles_ssd_histograms(path, results, num_buckets=50):
    ...
    lines.append(
        ...
        f"MESA R-R Intervals - all {num_files} files</text>"
    )
    ...
    for f_idx, fr in enumerate(results):
        ...
        ssd_vals = [
            round(b["rr_ssd"], 6) for b in all_bins if not np.isnan(b["rr_ssd"])
        ]
        ...
        lines.append(
            f'<text x="{ox + cell_w // 2}" y="{oy + 26}" '
            f'text-anchor="middle" class="stats">'
            f"n={len(ssd_vals)}  mean={mn:.6f}  med={md:.6f}</text>"
        )
...
def write_allfiles_rloc_histograms(path, results, num_buckets=50):
    ...
    lines.append(
        f'<text x="{svg_w // 2}" y="30" text-anchor="middle" class="title">'
        f"MESA R Locations - all {num_files} files</text>"
    )
    ...
    for f_idx, fr in enumerate(results):
        ...
        lines.append(
            f'<text x="{ox + cell_w // 2}" y="{oy + 26}" '
            f'text-anchor="middle" class="stats">'
            f"n={n_total_beats}  mean={mn:.6f}  med={md:.6f}</text>"
        )
...
def run_pair(pair_label, ids, load_a, load_b, out_dir, label_a, label_b):
    """Run one pairwise comparison across all subjects."""
    ...
    for file_id in ids:
        ...
        results.append(fr)
        write_per_file_csv(out_dir / f"{file_id}_wave_comparison.csv", fr)
    ...
    write_allfiles_ssd_histograms(out_dir / "allfiles_ssd_histograms.svg", results)
    write_allfiles_rloc_histograms(out_dir / "allfiles_ssd_histograms_rr.svg", results)
    write_combined_ssd_histogram(out_dir / "combined_ssd_histogram.svg", results)
...
def main():
    ...
    # Discover subjects in each source
    mira = {}
    ...
    # Loader factories. Each takes (file_id) and possibly (file_id, n_bins).
    def load_mira(fid, *_):
        return read_bin_file(mira[fid])

    def load_daniel(fid, *_):
        return read_mat_file(daniel[fid])

    def load_deep(fid, n_bins=0):
        return read_deep_folder(deep[fid], n_bins)
