...
# Number of pass-through channels appended at the end of each segment by the
# C++ writer (matches NUM_CHANNELS in 3_anneal_segments.cpp). The script
# doesn't use the pass-through data -- it just needs to skip past it to reach
# the next segment.
...
def _read_bin_contents(f, b, bin_index):
    ...
    def skip_signal():
        # Same on-disk shape as a regular signal block: uint64 count
        # followed by count doubles. Used to fast-forward past pass-through
        # upsampled blocks the script doesn't consume.
        raw = f.read(8)
        if len(raw) < 8:
            raise ValueError("Unexpected EOF in skip_signal")
        sz = struct.unpack("<Q", raw)[0]
        if sz > MAX_SANE_SIG:
            raise ValueError(f"skip_signal: count {sz} exceeds limit")
        f.seek(sz * 8, 1)

    def skip_raw_pairs():
        # Pass-through raw block: uint64 pair-count followed by
        # pair-count * 2 doubles (interleaved (t, v)).
        raw = f.read(8)
        if len(raw) < 8:
            raise ValueError("Unexpected EOF in skip_raw_pairs")
        n_pairs = struct.unpack("<Q", raw)[0]
        if n_pairs > MAX_SANE_IDX:
            raise ValueError(f"skip_raw_pairs: pair-count {n_pairs} exceeds limit")
        f.seek(n_pairs * 16, 1)
    ...
    # -- pass-through region: 41 x { upsampled signal, raw (t, v) pairs }
    # Appended by the C++ writer; this script doesn't consume the data,
    # just advances past it to land on the next segment's first byte.
