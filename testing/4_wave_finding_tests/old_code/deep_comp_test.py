...
and QTVI CSV zip files (_everyRRQTinputIntoEntropy_csv.zip).
"""
...
OUTPUT_DIR = Path(r"D:\USERS\MiraWelner\QTVI\testing\4_wave_finding_tests\results_csv")
...
# ============================================================================
# Read QTVI CSV zip (_everyRRQTinputIntoEntropy_csv.zip)
# ============================================================================


def _rel_sort_key(name):
    """Sort CSV filenames by their Rel/Abs numbers so beats are in order."""
...
def _parse_csv_inside_zip(zf, inner_name):
    """
    Parse one inner CSV: skip the header row, return arrays of
    time_sec (col 0) and rrint_sec (col 1).
    Format: comma-separated with leading spaces around values.
    """
    times, rrints = [], []
    ...
    return np.array(times), np.array(rrints)


def read_csv_zip_file(zip_path, bin_data):
    """
    Load all Rel*.csv files from the zip in sorted order, concatenate them,
    then split into per-bin lists aligned to the C++ bin boundaries
    (using each bin's ecg_bin_indexs to define the sample range).

    Returns a list of per-bin dicts with keys:
        ecgRIndex    -- R-peak sample indices (int, 0-based, time_sec * SR)
        ecgRIndex_f  -- same as float64
        ppgMinAmps   -- empty (not available in CSV)
        ecgSignal    -- empty
        ppgSignal    -- empty
    """
    empty = {
    ...
    if not Path(zip_path).exists():
        return [dict(empty) for _ in bin_data]
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        for name in inner_names:
            t, rr = _parse_csv_inside_zip(zf, name)
            all_times.append(t)
            all_rrints.append(rr)
            all_inner_names.append(name)

    if not all_times:
        return [dict(empty) for _ in bin_data]

    # Each inner CSV corresponds to one bin by position (Rel1=bin0, Rel2=bin1...).
    # Times in each CSV are seconds from that bin's epoch start.
    # C++ ecgRIndex is also 0-based within each bin (local, not absolute).
    # Direct conversion: local_sample = round(time_sec * SR).
    results = []
    for bin_idx, b in enumerate(bin_data):
        if bin_idx >= len(all_times) or len(all_times[bin_idx]) == 0:
            results.append(dict(empty))
            continue

        t_arr = all_times[bin_idx]
        local_samples = np.round(t_arr * SR).astype(np.int64)
...
def main():
    ...
    for bin_file in BIN_DIR.glob("*_wave_markings.bin"):
        ...
        # Glob for the zip rather than constructing exact name, in case
        # of minor naming variations
        zip_matches = list(
            CSV_DIR.glob(
                f"{file_id}_part1_ECG_fs1000_everyRRQTinputIntoEntropy_csv.zip"
            )
        )
        if zip_matches:
            ids.append((file_id, zip_matches[0]))

    # Diagnostic: show what was found
    print(
        f"  Found {len(list(BIN_DIR.glob('*_wave_markings.bin')))} bin files in BIN_DIR"
    )
    print(f"  Found {len(list(CSV_DIR.glob('*.zip')))} zip files in CSV_DIR")
    print(f"  Matched: {len(ids)} pairs")
    if not ids:
        print("  First few zips found:")
        for z in list(CSV_DIR.glob("*.zip"))[:3]:
            print(f"    {z.name}")
        print("  First few bins found:")
        for b in list(BIN_DIR.glob("*_wave_markings.bin"))[:3]:
            print(f"    {b.name}")
    ids.sort(key=lambda x: x[0])

    print("=" * 70)
    print("WAVE BOUNDS COMPARISON: C++ vs QTVI CSV")
    print("=" * 70)
    ...
    results = []
    for file_id, zip_path in ids:
        print(f"  {file_id}...", end="", flush=True)
        bin_data = read_bin_file(BIN_DIR / f"{file_id}_wave_markings.bin")
        csv_data = read_csv_zip_file(zip_path, bin_data)

        if not bin_data or not csv_data:
            print(" SKIPPED (empty data)")
            continue

        fr = compare_file(file_id, bin_data, csv_data)
        results.append(fr)
