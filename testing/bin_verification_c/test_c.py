#!/usr/bin/env python3
"""
verify_bin.py -- round-trip check for the uniform 36-channel .bin against its
original source (CHAOS .dat or EDF .edf).

  python verify_bin.py <file.bin> <source.dat|.edf> [--plot out.png]
...
and writes a per-channel plot: bin upsampled line + bin raw dots + source overlay.

No third-party deps except numpy (+ matplotlib only if --plot).
"""
...
def main(argv):
    ...
    plot_out=None
    for a in argv:
        if a=="--plot": plot_out="verify_plot.png"
        elif a.startswith("--plot="): plot_out=a.split("=",1)[1]
    if len(args)<1:
        return selftest()
    binp=args[0]; srcp=args[1] if len(args)>1 else None
    ...
    src = read_source(srcp) if srcp else None
    ...
    if plot_out: make_plot(binp,hdr,present,src,plot_out)
...
def selftest():
    import tempfile
    d=tempfile.mkdtemp()
    dat=os.path.join(d,"src.dat"); binp=os.path.join(d,"t.bin")
    n=1600
    ecg=np.sin(2*np.pi*10*np.arange(n)/256.0)*100+8000
    pleth=np.cos(2*np.pi*2*np.arange(n)/256.0)*50+2000
    with open(dat,"w") as f:
        f.write("Index,System TimeStamp UTC,NLS_NOM_ECG_ELEC_POTL_I,NLS_NOM_PULS_OXIM_PLETH,CSUM\n")
        for i in range(n):
            pl = f"{pleth[i]:.0f}" if i%4==0 else ""     # PPG sparse every 4 rows
            f.write(f"{i},20210606 12:30:{i%60:02d}.000,{ecg[i]:.0f},{pl},{i}\n")
    cols={l:v for l,v in read_dat_columns(dat)}
    ecg_v=cols["NLS_NOM_ECG_ELEC_POTL_I"]; ppg_v=cols["NLS_NOM_PULS_OXIM_PLETH"]
    # build a bin: ch0 timestamp ramp, ch1 ECG (raw=ecg_v), ch4 PPG (raw=ppg_v), rest sentinel
    epoch=1622982625110.0
    size_up=np.zeros(NCH,"<u4"); size_raw=np.zeros(NCH,"<u4")
    native=np.zeros(NCH,"<f4"); uprate=np.zeros(NCH,"<f4")
    blocks=[]
    def up_of(v, srN, dst):   # trivial repeat upsample for the fixture
        r=int(round(dst/srN)); return np.repeat(v, r)
    for c in range(NCH):
        if c==0:
            u=epoch+np.arange(2000)*1.0; r=np.column_stack([epoch+np.arange(2001)*1.0]*2)
            native[c]=1000; uprate[c]=1000
        elif c==1:
            u=up_of(ecg_v,256,1000); t=epoch+np.arange(ecg_v.size)*(1000/256)
            r=np.column_stack([t,ecg_v]); native[c]=256; uprate[c]=1000
        elif c==4:
            u=up_of(ppg_v,125,500); t=epoch+np.arange(ppg_v.size)*(1000/125)
            r=np.column_stack([t,ppg_v]); native[c]=125; uprate[c]=500
        else:
            u=np.array([SENT]); r=np.array([[SENT,SENT]]); native[c]=0; uprate[c]=0
        size_up[c]=u.size; size_raw[c]=r.shape[0]; blocks.append((u,r))
    with open(binp,"wb") as f:
        f.write(struct.pack("<III",1,NCH,30))
        f.write(size_up.tobytes()); f.write(size_raw.tobytes())
        f.write(native.tobytes()); f.write(uprate.tobytes())
        f.write(struct.pack("<I",1))
        for u,r in blocks:
            f.write(u.astype("<f8").tobytes()); f.write(r.astype("<f8").tobytes())
        f.write(np.array([SENT],"<f8").tobytes())
    print("(self-test: synthetic .dat + matching .bin)")
    rc=main(["verify_bin.py", binp, dat, "--plot="+os.path.join(d,"selftest.png")])
    return rc
