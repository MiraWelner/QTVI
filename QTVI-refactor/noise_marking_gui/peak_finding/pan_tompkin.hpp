...
namespace pan_tompkin_detail {
    ...
    inline vector<double> interp1_linear(const vector<double>& x, const vector<double>& v,
...
inline PanTompkinResult pan_tompkin(const vector<double>& ecg_input, double fs,
    int gr = 0, const std::string& fileID = "")
{
    using namespace pan_tompkin_detail;
            ...
            if (((double)locs[i] - last_qrs) >= round(MISSED_QRS_FACTOR * test_m)) {
                ...
                if (sb_start < sb_end) {
                    ...
                    for (int k = sb_start; k <= sb_end; ++k) {
                        if (ecg_m[k] > pks_temp) { pks_temp = ecg_m[k]; locs_temp = k; }
                    }
                    ...
                    if (pks_temp > THR_NOISE) {
                        ...
                        for (int k = sb_h_start; k <= sb_h_end; ++k) {
                            if (ecg_h[k] > y_i_t) { y_i_t = ecg_h[k]; x_i_t = k - sb_h_start; }
                        }
        ...
        if (pks[i] >= THR_SIG) {
            if (Beat_C >= 3) {
                ...
                if (((double)locs[i] - last_qrs_loc) <= round(TWAVE_REFRACTORY_SEC * fs)) {
                    ...
                    double s1 = 0, s2 = 0;
                    int n1 = 0, n2 = 0;
                    for (int k = slope1_start; k < slope1_end; ++k) { s1 += (ecg_m[k + 1] - ecg_m[k]); n1++; }
                    for (int k = slope2_start; k < slope2_end; ++k) { s2 += (ecg_m[k + 1] - ecg_m[k]); n2++; }
                    if (n1 > 0) s1 /= n1;
                    if (n2 > 0) s2 /= n2;

                    if (fabs(s1) <= fabs(TWAVE_SLOPE_RATIO * s2)) {
                        Noise_Count++;
                        if (Noise_Count > (int)nois_c.size()) { nois_c.resize(Noise_Count); nois_i.resize(Noise_Count); }
