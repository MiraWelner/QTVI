/**
 * @file   SegmentPPG.cpp
 * @brief  Takes a PPG signal, identifies the peaks and troughs, and then uses them to segment the
 *         PPG signal. The output is 2 index arrays, one with the peaks and the other with the troughts.
 *
 * @author Mira Welner
...
// ============================================================================
static pair<vector<size_t>, vector<size_t>> segBeats(const vector<double>& ppg, const vector<int>& mask);
static size_t newVallyFromPeaks(const vector<double>& ppg, size_t currVal, size_t currPeak,
...
SegmentPPGResult SegmentPPG(const vector<double>& ppg) {
    ...
    auto [peakidx, vallyidx] = segBeats(ppg, mask);
    ...
    for (size_t vidx = 0; vidx < vallyidx.size() - 1; ++vidx) {
        // Skip first and last valleys (boundary conditions)
        ...
        if (valley_outliers[vidx]) {
            vallyidx[vidx] = newVallyFromPeaks(ppg, vidx, pidx,
                peakidx, vallyidx, peak_outliers, valley_outliers);
        }
        pidx++;
    }
...
static size_t newVallyFromPeaks(const vector<double>& ppg, size_t currVal, size_t currPeak,
    ...
    // Only recompute if both surrounding peaks are clean
